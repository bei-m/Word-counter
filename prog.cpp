...
void irasymas(string &a,map<string, vector<int>> &zodziai, int line)
{
    ...
    for (int i=a.size()-1; i>-1; i--)
    {
        if (!isalpha(a.at(i)))
        {
            ...
            else if (a.at(i)=='\'') {a.erase(i, a.back()); break;}
            else {a.erase(i,a.back());}
        }
    }
    //IRASYMAS
    if (!b)
...
int main()
{
    ...
    //REGEX SUDARYMAS IESKOTI LINKAMS
        string x,word;
        ifstream galunes ("g.txt");
        vector<string> tlds;
        while (getline(galunes, x))
        {tlds.push_back(x);}
        std::string reg = "(?:\\.(?:[a-zA-Z0-9-]+\\.)?)(?:";
        for (const auto& tld : tlds) 
        {reg += tld + "|";}
        tlds.clear(); tlds.shrink_to_fit();
        reg.pop_back(); reg += ")\\b";
        regex pattern(reg);
        galunes.close();
    ...
    fill_n(back_inserter(skirtukas), ((ilgis_z+3+12+(ilgis_l+1)*12-8))/2, '=');
    ...
    skirtukas.clear();
    fill_n(back_inserter(skirtukas), ((ilgis_z+(ilgis_l+1)*12-11+1)/2), '=');
    ...
    skirtukas.clear();
    fill_n(back_inserter(skirtukas), (ilgis_z+3+12+(ilgis_l+1)*12), '-');
    rez<<skirtukas<<endl;
    int q;
    string skirtukas2;
    for (auto &i: zodziai)
